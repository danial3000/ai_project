...
#..................................
def get_action_rewards(env):
    """
    Creates a 64x4 matrix representing the reward for each action at every state.
    """
    grid_size = env._UnknownAngryBirds__grid_size
    action_rewards = np.zeros((grid_size * grid_size, 4))  # 64 states, 4 actions

    for i in range(grid_size):
        for j in range(grid_size):
            state_index = i * grid_size + j  # Linear index for state (i, j)
            env._UnknownAngryBirds__agent_pos = (i, j)  # Set agent position

            for action in range(4):  # Iterate over actions: 0=Up, 1=Down, 2=Left, 3=Right
                initial_pos = env._UnknownAngryBirds__agent_pos
                next_state, reward, _, _ = env.step(action)
                action_rewards[state_index, action] = reward
                env._UnknownAngryBirds__agent_pos = initial_pos  # Reset position
...
class DQLearning:
    def __init__(self, env, learning_rate=1e-3, discount_factor=0.99, epsilon_start=1.0,
        ...
        #............
        # Get action rewards for all states in the environment
        self.action_rewards = get_action_rewards(self.env)
        #...............
    ...
    def select_action(self, state_vector):
        ...
        def select_action(self, state_vector):
            ...
            if random.random() < self.epsilon:
                ...
                state_index = np.argmax(state_vector)  # پیدا کردن خانه فعلی عامل
                with torch.no_grad():
                    q_values = self.q_network(state_tensor)
                    action_rewards = torch.tensor(self.action_rewards[state_index], dtype=torch.float32).to(self.device)
                    adjusted_q_values = q_values + action_rewards
                return torch.argmax(adjusted_q_values).item()  # Exploit: best action considering rewards
    ...
    def train_episode(self):
        ...
        while not done:
            ...
            state_index = agent_pos[0] * self.env._UnknownAngryBirds__grid_size + agent_pos[1]
            reward += self.action_rewards[state_index, action]
            # ..............
